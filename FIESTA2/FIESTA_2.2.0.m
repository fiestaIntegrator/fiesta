...
If[temp[[1]]==={},Return[{xx}]];
   If[ExpandMode,temp={Append[temp[[1]],2],AppendTo[temp[[2]],(*If[Power2,z+2z2,z]*)Exponent[temp[[3]][[1]],ExV]],temp[[3]]/.ExV->1,temp[[4]],temp[[5]],temp[[6]]}];
...
,30,    Print[vardegrees];Print[xx];Print[x0];Print[maxorder];Abort[]
];
    Return[temp];
]
...
(*    temp=DeleteCases[DeleteCases[DeleteCases[Variables[ZZ],x[_]],ep],ExpandVariable];
   ...
   (* Print[ExpandVariable];
    ...
    n=Length[intvars];
        If[NumberOfSubkernels>0,
            DistributeDefinitions[ResolutionMode,UsingQLink,HASZ,ExpandOrder,ExV,ZREPLACEMENT,n,x,y,Power2,ExpandDirect,ExpandMode];
        ];
    Terms=0;
...
(*Print["Part ",num,": taking residue in z1 = ",res1[[1]]," with coefficient ",res1[[2]]];*)
			    res1taken=MyResidue[temp[[2]],res1[[2]]*temp[[3]],res1[[1]],res1[[3]]];
...
SDEvaluateDirect[var_, functions_, degrees_, order_] :=  Module[{temp, n, ZZ, result, deltas, intvars,i},
        If[GraphInput,GraphUsed=True,GraphUsed=False,GraphUsed=False];
            GraphInput=False;
    ExpandMode=False;
    DMode=False;
