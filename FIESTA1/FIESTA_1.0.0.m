...
VersionString:="FIESTA, version 1.0.0";
...
DoIntegrate[xx__] := Module[{temp2,vars, temp,i,rules,met,res},
vars = Union[Cases[xx,y[aaa_],Infinity],Cases[xx,x[aaa_],{0,Infinity}]];
...
KillNegTerms[ZZ_]:=Module[{temp,vars,subsets,U,F,UF,Z,i},
    ...
    For[i=1,i<=Length[subsets],i++,
        If[And[Length[NegTerms[VarDiffReplace[UF[[2]],subsets[[i]]]]]<Length[temp],
            ...
            Z={#[[1]]/2,#[[2]]}&/@Z;
            Return[{VarDiffReplace[{Z,M,UF},subsets[[i]]],VarDiffReplace[{Z,M,UF},{subsets[[i]][[2]],subsets[[i]][[1]]}]}]
        ]; 
    ];
    Return[{ZZ}]
]

UseEpMonom[ZZ_]:=Module[{temp,vars,subsets,U,F,Z,M,i,UF},
    {Z,M,UF}=ZZ;
    M=Expand[M];
    If[And[Head[M]===Power,Head[Expand[M[[1]]]]===Plus],
        {{#[[1]],Prepend[#[[2]],M[[2]]]}&/@Z,Prepend[UF,M[[1]]]}
    ,
        {{#[[1]]*M,#[[2]]}&/@Z,UF}        
    ]
]
...
PrepareString[i_]:=Module[{temp,num},
                        If[i===CurrentOrder,Dots[],gi++];CC[100];
                        temp=ToExpression[QGet[DataPath<>"2/",ToString[i]<>"-"<>ToString[StringPrepareCounter]]];
                        num=temp[[1]];
                        temp=temp[[2]];
                        ...
                        If[Not[temp[[2]]===0],
                             ...
                             NumberOfVariables[TargetDatabase]=Max[NumberOfVariables[TargetDatabase],vars[[2]]];
                             PSector[TargetDatabase,num]=PSector[TargetDatabase,num]+1;
                             QPut[RemoveFIESTAName[DataPath<>ToString[TargetDatabase]<>"/"],ToString[num]<>"-"<>ToString[PSector[TargetDatabase,num]],MyString[temp]<>";\n"];
                        ];
...
Module[{Z,U,F,SD,f,forsd,ii,i,vars,n,m,j,l,rule,Z3,Z2,U2,F2,coeff,k,Jac,result,md,result2,pol,active,zsets,SDCoeffs,timecounter},
    NoNan=True;
...
(*            Print[j];*)
            ...
            If[STRATEGY===STRATEGY_0,
                SD[[j]]={IdentityMatrix[n]}
                ,   
                If[STRATEGY===STRATEGY_X,
                    pol=Expand[##/.rules]&/@U;
                    active=(Union@@(Sort[Variables[##]]&/@pol));
                    SD[[j]]=FindSD[Degrees[##,active]&/@pol];
                    active=##[[1]]&/@active;
                    SD[[j]]=BlockMatrix[SD[[j]],active,n];
                    ,
                    pol=Expand[Times@@U]/.rules;
                    active=##[[1]]&/@Sort[Variables[pol]];
                    SD[[j]]=FindSD[{Degrees[pol]}];
                    SD[[j]]=BlockMatrix[SD[[j]],active,n];
                ]
            ];
        ];
    ...
    ff=Reap[
        ...
        For[j=1,j<=Length[SD],j++,
            ...
            If[SDCoeffs[[j]]===0,Continue[]];
            rules=Rule[x[##],1]&/@zsets[[j]];
            For[k=1,k<=Length[SD[[j]]],k++,
                If[Not[ForceMixingOfSectors],SCounter++];
                reps=Inner[Power[#2, #1] &, Transpose[SD[[j]][[k]]], Array[y,n],Times];
                ...
                For[ii=1,ii<=Length[Z2],ii++,
                    ...
                    f=f//.y->x;                    
                    If[UsingQLink,
                        If[Not[f[[3]]===0],QPut[DataPath<>"3/",ToString[run3,InputForm],ToString[{If[ForceMixingOfSectors,j,SCounter],f},InputForm]];run3++]
                    ,
                        Sow[{If[ForceMixingOfSectors,j,SCounter],f}];                        
                    ];
                ];
            ];
        ];
    ...
    If[UsingQLink,
        ...
        RawPrintLn[MyTimingForm[AbsoluteTiming[
            For[run3=1;run2=1,run3<=DLength[3],run3++,
                Dots[];CC[100];
                temp=ToExpression[QGet[DataPath<>"3/",ToString[run3]]];
                num=temp[[1]];
                temp=ExpandZ[temp[[2]]];
                (QPut[DataPath<>"2/",ToString[run2,InputForm],ToString[{num,##},InputForm]];run2++)&/@temp;
            ];
            DLength[2]=run2-1;
        ][[1]]]," seconds"];
 ...
 (*       ClearDatabase[1];*)
        ...
        RawPrintLn[MyTimingForm[AbsoluteTiming[
            For[run2=1;(*run1=1*),run2<=DLength[2],run2++,
                Dots[];CC[100];temp=ToExpression[QGet[DataPath<>"2/",ToString[run2]]];
                num=temp[[1]];
                temp=temp[[2]];
                temp={temp[[1]],temp[[2]],temp[[3]]*temp[[4]]};
                temp=EpNegExpand[temp];
                (QPut[DataPath<>"1/",ToString[run1,InputForm],ToString[{num,##},InputForm]];run1++)&/@temp;
            ];
    (*        DLength[1]=run1-1;*)
...
(*        ff=GroupTerms[ff];*)
  ...
  If[Not[UsingQLink],
    ...
    If[UsingQLink,
        ...
        RawPrintLn[MyTimingForm[AbsoluteTiming[ 
            For[run1=1,run1<=DLength[1],run1++,
                Dots[];CC[100];
                temp=ToExpression[QGet[DataPath<>"1/",ToString[run1]]];
                num=temp[[1]];
                temp=temp[[2]];
                QPut[DataPath<>"1/",ToString[run1],ToString[{num,ConstructTerm[temp]},InputForm]]
            ];
        ][[1]]]," seconds."];     
        ...
        RawPrintLn[MyTimingForm[AbsoluteTiming[ 
            For[run1=1,run1<=DLength[1],run1++,
                ...
                num=temp[[1]];
                temp=temp[[2]];
                Dots[];CC[100];QPut[DataPath<>"1/",ToString[run1],ToString[{num,AdvancedFirstVars[temp]},InputForm]]
            ];
        ][[1]]]," seconds."];   
        ...
        RawPrintLn[MyTimingForm[AbsoluteTiming[
            For[run1=1,run1<=DLength[1],run1++,
                ...
                (TermNumber[##[[1]]]=TermNumber[##[[1]]]+1;
                    min=Min[min,##[[1]]];
                    QPut[DataPath<>"2/",ToString[##[[1]]]<>"-"<>ToString[TermNumber[##[[1]]]],ToString[{num,{temp1,##[[2]]}},InputForm]]
                )&/@temp2;
            ];
        ][[1]]]," seconds."];          
    ,
        ...
        PrepareDots[Length[ff]];
        RawPrintLn[MyTimingForm[AbsoluteTiming[   
            ff=({##[[1]],Dots[];CC[100];ConstructTerm[##[[2]]]})&/@ff;     
        ][[1]]]," seconds."];   
        RawPrint["Replacing variables"];
        PrepareDots[Length[ff]];
        RawPrintLn[MyTimingForm[AbsoluteTiming[   
            ff=({##[[1]],Dots[];CC[100];AdvancedFirstVars[##[[2]]]})&/@ff;     
        ][[1]]]," seconds."]; 
        RawPrint["Epsilon expansion"];
        PrepareDots[Length[ff]];
        RawPrintLn[MyTimingForm[AbsoluteTiming[
            fff=Flatten[(Dots[];CC[100];
                    num=##[[1]];
                    temp=##[[2]];
                    temp1=temp[[1]];
                    temp2=EpPosExpand[Expand[temp[[2]]],order];
                    {num,{##[[1]],{temp1,##[[2]]}}}&/@temp2)&/@ff,1];
        ][[1]]]," seconds."];
    ...
    For[CurrentOrder=min,CurrentOrder<=order,CurrentOrder++,
        ...
        If[UsingC,
            ...
            ,
                ...
                PrepareDots[Length[ff]];
                RawPrintLn[MyTimingForm[AbsoluteTiming[
                       ff=({##[[1]],Dots[];CC[100];Expand[##[[2]]]})&/@ff;
                ][[1]]]," seconds."];  
                If[IfCut>0,
                    RawPrint["Making if conditions"];
                    PrepareDots[Length[ff]];
                    RawPrintLn[MyTimingForm[AbsoluteTiming[
                        ff=({##[[1]],Dots[];CC[100];MakeIfConditions[##[[2]]]})&/@ff;
                    ][[1]]]," seconds."];  
                ]; 
                ff={##[[1]],##[[2]][[2]]}&/@ff;   
                (*ff=Plus@@ff;
                If[Head[ff]===Plus,ff=List@@ff,ff={ff}];                *)
                RawPrint["Counting variables: "];
                PrepareDots[Length[ff]];
                RawPrintLn[MyTimingForm[AbsoluteTiming[
                    NumberOfVariables=Max@@((CountVars[##[[2]]][[2]])&/@ff);
                ][[1]]]," seconds."];   
...
SDEvaluate[{U_,F_,h_},degrees1_,order_]:=Module[{ZZ,n,degrees,epdegrees,runorder,outside,a1,deltas},
    ...
    epdegrees=Expand[degrees1-(degrees1/.{ep->0})]/ep;
    a1=Plus@@degrees1;
    ZZ={{{1,{a1-(h+1)(2-ep),-(a1-h(2-ep))}}},{U,F}};
    runorder=order;
    If[((a1-(h)(2-ep))/.ep->0)<=0,runorder++];
    For[j=1,j<=Length[degrees],j++,
		If[And[degrees[[j]]<=0,Not[epdegrees[[j]]==0]],runorder--]
    ];
    outside=(E^(h EulerGamma ep))*Gamma[a1-h(2-ep)];
    deltas={Range[n]};
    {ZZ,deltas,degrees,epdegrees}=KillNegativeIndices[ZZ,deltas,degrees,epdegrees];
...
Degrees[pol_]:=Degrees[pol,Sort[Variables[pol]]];

Degrees[pol_,var_] := Module[{rule, temp, degrees, i},
(*    var=Array[x,n];*)
  rule = Rule[##, 1] & /@ var;
  temp = List @@ Expand[pol];
...
MakeOneStep[xx_,facet_]:=Module[{res,n}, 
...
n=Length[xx[[1]][[1]]]; 
  ...
  For[i = 1, i <= Length[active], i++,
        ...
        AppendTo[res, {(Matrix.##) & /@ xx[[1]],Matrix.xx[[2]]}];
  ];
  res = {NormShift[#[[1]]],#[[2]]}&/@ res;
  res = {OnlyLowPoints[#[[1]]],#[[2]]}&/@ res;
    res
];
...
TriLower[xx_, yy_] := 
 ...
 If[STRATEGY==STRATEGY_S,
  n=Length[xx[[1]][[1]]]; 
  facet = TimeConstrained[Facet[xx[[1]]],100,RawPrintLn[xx[[1]]];Abort[]];
  res=MakeOneStep[xx,facet];
  If[Not[And@@((Length[##[[1]]]<Length[xx[[1]]])&/@res)],
      {m0,l0,n0,vmin,vmax}=MLN[xx[[1]]];
      If[Not[And@@(TriLower[MLN[##[[1]]],{m0,l0,n0}]&/@res)],
       ...
       For[m=2,m<=n,m++,
            ...
            For[i=1,i<=Length[sbs],i++,
                ...
                If[And@@((Not[(facet.##)===0])&/@(xxx[[1]][[j]])),
                    res=MakeOneStep[##,facet]&/@({##,xxx[[2]]}&/@(xxx[[1]]));  
                    res=Transpose[res];
                    res=Transpose/@res;
                    res={##[[1]],##[[2]][[1]]}&/@res;                    
                    Return[res]
...
FindSD[xxx_]:=FindSD[xxx,IdentityMatrix[Length[xxx[[1]][[1]]]]]

FindSD[xxx_,Matrix_] := FindManySD[{{xxx,Matrix}}];

FindManySD[xxx_] := Module[{temp,i}, yy = {};
  xx = {OnlyLowPoints/@(##[[1]]),##[[2]]}&/@xxx;
  temp=xx;
